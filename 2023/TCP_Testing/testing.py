...
def receive_exactly(sock, amount, msg, expect, decode=True):
    ...
    try:
        ...
        print(f"Error receiving acknowledge {msg}: {err}")
    ...
    while len(data) < amount:
        try:
            ...
            print(f"Error receiving {msg}: {err}")
    return data
...
def check_error(data, expect):
    ...
    if dat == RESPONSE_ERROR_MAGIC:
        print("Error: Invalid magic byte")
    ...
    if dat == RESPONSE_ERROR_AUTH_INVALID:
        print("Error: Authentication invalid. Is the password correct?")
    ...
    if dat == RESPONSE_ERROR_UNKNOWN:
        print("Unknown error from ESP")
    if not isinstance(expect, (list, tuple)):
        expect = [expect]
    if dat not in expect:
        print(f"Unexpected response from ESP: 0x{data[0]:02X}")
...
def perform_ota(
    ...
    if version not in (OTA_VERSION_1_0, OTA_VERSION_2_0):
        print(f"Unsupported OTA version {version}")
...
def run_ota(remote_host, remote_port=3232):
  return run_ota_impl_(remote_host, remote_port)